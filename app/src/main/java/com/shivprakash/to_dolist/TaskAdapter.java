...
public class TaskAdapter extends RecyclerView.Adapter<TaskAdapter.TaskViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TaskViewHolder holder, int position) {
        ...
        setPriorityColor(holder.text_priority, currentItem.getPriority());

        holder.taskCard.setCardElevation(0);
        holder.taskCard.setUseCompatPadding(false);
    ...
    private void setPriorityColor(TextView view, String priorityRaw) {
        if (view == null) return;
        String p = priorityRaw == null ? "" : priorityRaw.trim();

        if (p.equalsIgnoreCase("High")) {
            view.setTextColor(view.getResources().getColor(R.color.priorityHigh));
        } else if (p.equalsIgnoreCase("Medium")) {
            view.setTextColor(view.getResources().getColor(R.color.priorityMedium));
        } else if (p.equalsIgnoreCase("Low")) {
            view.setTextColor(view.getResources().getColor(R.color.priorityLow));
        } else {
